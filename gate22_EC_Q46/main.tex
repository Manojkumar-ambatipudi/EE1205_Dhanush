...
\begin{figure}[H]
    \resizebox{0.34\textwidth}{!}{\input{figs/question_blockdiag}}
    \caption{Block Diagram of Systems}
    \label{fig:question_fig}
\end{figure}
\hfill(GATE22 EC Q46)\\
\solution 
\input{tables/gate22_EC_Q46tab}
\begin{figure}[H]
    \resizebox{0.55\textwidth}{!}{\input{figs/lti_system_diag}
    }
...
\end{align}
$x(t)=\sin\brak{t}$, and $w_{o}=1 \frac{rad}{sec}$
\begin{align}
...
\begin{align}
    Y\brak{\omega}=&\brak{\delta(\omega+\omega_0)-\delta(\omega-\omega_0)}\abs{H\brak{\omega}}e^{j\Phi\brak{\omega}}\label{eq:gate22_ec_q46.1}
\end{align}
...
\begin{align}
    Y\brak{\omega}=&j\abs{H(-\omega_0)}e^{j\Phi(-\omega_0)}\delta(\omega+\omega_0) \\&- j\abs{H\brak{\omega_0}}e^{j\Phi(j\omega_0)}\delta(\omega-\omega_0) \notag
\end{align}

Given $h(t)$ a real-time domain signal, $H\brak{\omega}$ is conjugate symmetric.
\begin{align}
    H\brak{\omega}=H(-\omega)\\
    \Phi(-\omega_0)=-\Phi\brak{\omega_0}
\end{align}
Therefore,
 \begin{align}
    Y\brak{\omega}= j\abs{H\brak{\omega_0}}\brak{e^{-j\Phi\brak{\omega_0}}\delta(\omega+\omega_0) - e^{j\Phi\brak{\omega_0}}\delta(\omega-\omega_0)}
\end{align}
...
\begin{align}
    ...
    &\implies y(t) =\abs{H\brak{\omega_0}}\sin{\brak{\omega_0t+\Phi\brak{\omega_0}}} \label{eq:gate_ec_q46_finaloutput}
\end{align}
...
$S_{3}$ and $S_{4}$ cannot be LTI system.
\end{document}
